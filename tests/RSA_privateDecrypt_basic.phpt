...
try {
	...
	echo $e->getCode() === LCrypto\RSAException::PRIV_DECRYPT_INPUT_LONG ? "INPUT LONG\n" : "BAD CODE\n";
}
...
try {
	$rsa1->privateDecrypt($ctext1_ex, LCrypto\RSA::PADDING_X931);
} catch (LCrypto\RSAException $e) {
...
function rsa_test_public_crypt($i, $rsa, $ptext_ex, $ctext_ex) {
	$ctext1_pkcs1_1_5 = $rsa->publicEncrypt($ptext_ex, LCrypto\RSA::PADDING_PKCS1);

    if (strlen($ctext1_pkcs1_1_5) !== strlen($ctext_ex)) {
        echo "KEY $i: PKCS#1 v1.5 encryption failed\n";
    }

	$ptext1_pkcs1_1_5 = $rsa->privateDecrypt($ctext1_pkcs1_1_5, LCrypto\RSA::PADDING_PKCS1);
	...
	$ctext1_pkcs1_oaep = $rsa->publicEncrypt($ptext_ex, LCrypto\RSA::PADDING_OAEP);

    if (strlen($ctext1_pkcs1_oaep) !== strlen($ctext_ex)) {
        echo "KEY $i: OAEP encryption failed\n";
    }

	$ptext1_pkcs1_oaep = $rsa->privateDecrypt($ctext1_pkcs1_oaep, LCrypto\RSA::PADDING_OAEP);
